...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = `Evaluate the following listening comprehension answers based on the transcript. Provide:
...
Provide your evaluation in a structured format.`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are an IELTS listening test evaluator. Provide fair, constructive feedback and accurate scoring.",
        },
        {
          role: "user",
          content: prompt,
